...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForTargetKeyID(ctx, apiReader, namespace, ko); err != nil {
...
func validateReferenceFields(ko *svcapitypes.Alias) error {
	...
	if ko.Spec.TargetKeyRef != nil && ko.Spec.TargetKeyID != nil {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("TargetKeyID", "TargetKeyRef")
	}
	if ko.Spec.TargetKeyRef == nil && ko.Spec.TargetKeyID == nil {
		return ackerr.ResourceReferenceOrIDRequiredFor("TargetKeyID", "TargetKeyRef")
	}
	return nil
}

// resolveReferenceForTargetKeyID reads the resource referenced
// from TargetKeyRef field and sets the TargetKeyID
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForTargetKeyID(
	ctx context.Context,
	apiReader client.Reader,
	namespace string,
	ko *svcapitypes.Alias,
) (hasReferences bool, err error) {
	if ko.Spec.TargetKeyRef != nil && ko.Spec.TargetKeyRef.From != nil {
		hasReferences = true
		arr := ko.Spec.TargetKeyRef.From
		if arr.Name == nil || *arr.Name == "" {
			return hasReferences, fmt.Errorf("provided resource reference is nil or empty: TargetKeyRef")
		}
		obj := &svcapitypes.Key{}
		if err := getReferencedResourceState_Key(ctx, apiReader, obj, *arr.Name, namespace); err != nil {
			return hasReferences, err
		}
		ko.Spec.TargetKeyID = (*string)(obj.Status.KeyID)
	}
